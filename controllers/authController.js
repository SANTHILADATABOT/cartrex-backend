    ...
    if (existingUser && existingUser.verifyuser === "verified") {
      ...
      return res
        ...
        .json({
          ...
          state: { data: email, step: "verification", process: "verification" },
        });
    }
...
exports.login = async (req, res) => {
  try {
    ...
    if (role === "admin") {
      account = await AdminUser.findOne({ "personalInfo.email": email }).select(
        "+password"
      );
  ...
  try {
    ...
    if (!user)
      return res
        ...
        .json({
          success: false,
          message: "Please enter a verified registered email",
        });
